...
void GetTimeZoneDisplayName_FromCalendar(
    const char* locale,
...
void GetTimeZoneDisplayName_FromPattern(
    const char* locale,
    const UChar* timeZoneId,
    const UDate timestamp,
    const UChar* pattern,
...
void FixupTimeZoneGenericDisplayName(
    const char* locale,
    const UChar* timeZoneId,
    const UDate timestamp,
    UChar* genericName,
            ...
            if (u_strcmp(testStandardName, standardName) == 0)
            {
                ...
                if (u_strlen(testGenericName) < originalGenericNameActualLength)
                {
                    ...
                    if (u_strcmp(testGenericName, testGenericLocationName) != 0)
                    {
                        ...
                        u_strcpy(genericName, testGenericName);
                        break;
                    }
                }
            }
