                                                      ...
                                                      int32_t resultLength)
{
    ...
    switch (type)
    {
        ...
        case TimeZoneDisplayName_Generic:
            ...
            FixupTimeZoneGenericDisplayName(locale, timeZoneId, timestamp, result, resultLength, &err);
            break;
...
void GetTimeZoneDisplayName_FromCalendar(const char* locale,
    ...
    UErrorCode* err)
{
    // Use the ICU Calendar API to get the standard and daylight display names.
...
void GetTimeZoneDisplayName_FromPattern(const char* locale,
    ...
    UErrorCode* err)
{
    // Use the ICU Date Formatting API to get the various forms of generic time zone names using patterns.
    ...
    UErrorCode* err)
{
    ...
    UChar standardName[256];
    UCalendar* calendar = ucal_open(timeZoneId, -1, locale, UCAL_DEFAULT, err);
    ucal_setMillis(calendar, timestamp, err);
    ucal_getTimeZoneDisplayName(calendar, UCAL_STANDARD, locale, standardName, 256, err);

    // See if the generic name is the same as the standard name.
    if (u_strcmp(result, standardName) == 0)
    {
        // Get some details for later comparison.
        const int32_t originalGenericNameActualLength = u_strlen(result);
        const int32_t baseOffset = ucal_get(calendar, UCAL_ZONE_OFFSET, err);

        // Allocate some additional strings for test values.
        UChar testTimeZoneId[64];
        UChar testStandardName[256];
        UChar testGenericName[256];
        UChar testGenericLocationName[256];
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            GetTimeZoneDisplayName_FromCalendar(locale, testTimeZoneId, timestamp, UCAL_STANDARD, testStandardName, 256, err);

            // See if the test time zone has the same standard name.
            if (u_strcmp(testStandardName, standardName) == 0)
            {
                // Get the generic name from the test time zone.
                GetTimeZoneDisplayName_FromPattern(locale, testTimeZoneId, timestamp, GENERIC_PATTERN_UCHAR, testGenericName, 256, err);

                // See if the test time zone has a shorter generic name.
                if (u_strlen(testGenericName) < originalGenericNameActualLength)
                {
                    // Just to be safe, make sure the test zone isn't using a generic name that is specific to a
                    // particular location.  For example, "Antarctica/Troll" uses "Troll Time" as
                    // a generic name, but "Greenwich Mean Time" as a standard name.  We don't
                    // want other zones that use "Greenwich Mean Time" to be labeled as "Troll Time".

                    GetTimeZoneDisplayName_FromPattern(locale, testTimeZoneId, timestamp, GENERIC_LOCATION_PATTERN_UCHAR, testGenericLocationName, 256, err);

                    if (u_strcmp(testGenericName, testGenericLocationName) != 0)
                    {
                        // We have found a better generic name.  Use it.
                        u_strcpy(result, testGenericName);
                        break;
                    }
                }
            }
